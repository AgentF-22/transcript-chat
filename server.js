...
// Create tables and ensure admin exists
async function initDB(){
  ...
  await pool.query(`
    INSERT INTO users (username, password, is_admin, blocked)
...
// deliverables stay in memory (they're temporary by design)
...
app.post('/api/login', async (req, res) => {
  ...
  res.json({ ok: true, token, isAdmin: !!user.is_admin });
});
...
app.post('/api/admin/approve/:id', requireAdmin, async (req, res) => {
  ...
  const r = rows[0];
  await pool.query('INSERT INTO users (username,password,is_admin,blocked) VALUES ($1,$2,FALSE,FALSE) ON CONFLICT (username) DO NOTHING', [r.username, r.password]);
...
app.get('/api/admin/users', requireAdmin, async (req, res) => {
  const { rows } = await pool.query('SELECT username, blocked, created_at FROM users WHERE is_admin=FALSE ORDER BY created_at ASC');
  res.json({ users: rows.map(r => ({ username: r.username, blocked: r.blocked, createdAt: r.created_at })) });
});
...
app.get('/api/admin/conversations', requireAdmin, async (req, res) => {
  ...
  // Mark replies from users as read
